...
具体的には `references/safety-policy.md` の**「サブエージェント契約（逐語コピー用）」ブロックを
そのままプロンプトの先頭に貼り**、貼ったことを `run.log` に記録します。
...
**`--only` / `--from` と `sast.enabled: true` の組み合わせ**：`--from`/`--only` でも新しい run-id で
出力先を作るので、前 run の成果物（`target-map.md` 等）を持ち込んで再開する場合は、**その
`target-map.md` 冒頭に記録された SAST run-id と、工程0 で解決した SAST run-id が一致することを
確認**してください。食い違えば停止します（別 run の分母と指摘が黙って混ざるため）。工程6・7 から
再開するのに `target-map.md` が無ければ、工程2 をやり直す（＝抽出サブエージェントを回し直す）。
...
3. **`sast.enabled: true` なら、ここで SAST 成果物を解決して run-id を固定する**
   （`references/sast-handoff.md`、安全の側は `references/safety-policy.md`「SAST 成果物の取り込み」）。
   - `report: latest` → `reports/sast/latest.json` を読む。無ければ**停止**し、`report:` に run
     ディレクトリを明示するよう案内する（SAST 側で出力先を既定から変えている場合に起きる）。
     `latest.json` の `path` 値にも「リポジトリ内の相対パス」の検査を掛ける。
   - **解決は1回だけ。** ここで確定した run-id を工程2・6・7 で使い回す（後で再解決しない。
     その間に SAST が走れば別 run を掴む）。
   - `attack-map.md` と `report-04.md` の**両方が存在すること**を確認する。片方でも欠けていれば
     **停止**（SAST が途中で止まった run を掴んでいる）。**完了判定はここでは行わない** —
     書きかけの文書は工程2 の子が「文書として完結していない」と差し戻す。
   - **対象照合**：attack-map の対象プロファイル（解析対象パス・アプリ種別・フレームワーク・
     主要言語・エントリポイントのファイル名・規模）を、工程1 で読んだ実物と突き合わせる。
     食い違えば**停止**（別対象の結果を食う事故）。**この照合は弱い**ことを承知しておくこと——
     解析対象パスは既定運用ではどのリポジトリでも「リポジトリ直下」相当になるので、
     `reports/sast/` を他所からコピーして持ち込まれた場合は**フレームワークやエントリポイントの
     一致で判断するしかない**。決定的な検出は工程4（SAST 由来だが実物に存在しない URL の件数）だが、
     **そちらは既にパケットを撃った後**なので、ここで弾けるものは弾く。
   - **縮退・古さでは止めない**（`safety-policy.md` の停止表）。使用した run-id を記録する。
4. 設定が不足・不十分な場合：**推測せず、Active Scanを開始しない。** 不足項目を列挙する。
   ギャップが安全に関わるなら停止する。`dast.yaml` が存在しない場合は、上記「設定生成」に従って
   生成を提案する（断られたら既定値で続行）。
5. チェックポイントのサマリを出力して継続する。
...
**`sast.enabled: true` でも、この工程は縮約しない。** attack-map は工程1 の**置換ではなく補強**で、
診断対象マップは両者の和集合になる（理由は `references/safety-policy.md`「SAST 成果物の取り込み」の
停止表の注記＝置換すると SAST 側の縮退がそのまま DAST の分母の縮退になる）。加えて、attack-map が
構造的に持てないものはこの工程でしか埋まらない：

- **実行環境の事実** — 実際の `base_url`／ポート／起動方法（attack-map は静的な宣言までしか知らない）。
- **外部通信の宛先の解決値** — SMTP ホスト、Webhook URL、決済先、stub/キャッチャの有無
  （`.env.example`・`docker-compose*`・環境変数由来。SAST は `.env` を読まないので持っていない）。
  **8C の判定は送る前に静的に決めるしかない**ため、ここが唯一の一次情報になる。
- **工程2.5 向けの動的な指標** — ログイン成功指標・ログアウト状態指標の候補（実応答でしか決まらない）。
- **SAST 実行後の差分** — 古い attack-map を使う場合、ルート定義が変わっていないか。
...
要否＋必要な権限／**認可の実態**（認証要否の2値に落ちない状態を潰さない）／入力箇所＋想定データ形式／
セキュリティ上の注目点・想定脆弱性／優先度（**順序であって絞り込みではない**）／根拠
（ソース由来か・推測か・**伝聞 `[H]` か**）＋未確認事項。

`reports/dast/<run-id>/target-map.md` に保存。チェックポイント。

**`sast.enabled: true` のとき**：工程0 で固定した SAST run から攻撃面と指摘を取り込む。
**抽出専任のサブエージェント2体に、順番に委譲する**（`references/sast-handoff.md`）——
子1 が `attack-map.md` を読んで `target-map.md` を書き、続いて子2 が**その `target-map.md` と**
`report-04.md` を読んで同じファイルに追記・更新する。並列にして親が突き合わせる形は採らない
（突合キーが揃わず、親が両方の全文を抱えることになる）。

- **起動のたびに `references/safety-policy.md` の「サブエージェント契約（逐語コピー用）」ブロックを
  そのままプロンプト先頭に貼り、貼ったことを `run.log` に記録する**（共通則 §6）。子は送信せず、
  渡された2本以外を読まず、安全の判断をしない。
- **親はどちらの子の入力も全文読まない。** 行うのは検算（attack-map の件数節／report-04 の
  件数表）と、合わなかった点の差し戻しだけ。子には「合った/合わない」ではなく
  **「どこがどう合わなかったか」**を返させる（検算材料自体が誤っていることがある）。
- 子が**「文書として完結していない」**（指定の節が見つからない／表が途中で切れている）と
  差し戻したら、**その run は使えないものとして停止**する（`references/safety-policy.md`）。
  SAST は完了マーカーを残さないので、途中で止まった run を掴むのはここでしか気づけない。
- 取り込んだ行・指摘には出自ラベル **`[H]`（伝聞＝SAST 由来）** と SAST の指摘 ID を付ける。
  DAST 自身は SAST の文書を読んでいないので、`[S]`（この run で確認）に混ぜない。
- **優先度は順序にだけ使う。** 撃つかどうかには使わない（工程6 の完了条件は全エントリに掛かる）。
...
1. ソース抽出URLと、ZAPが実際に到達したURLを比較し、未到達を分類する（ログイン必須／JS操作
   ...
   **`sast.enabled: true` のときは分類肢に「SAST 由来だが実物に存在しない（404／未登録）」を足す。**
   これは到達手段の問題ではなく**分母が古い／別対象を指している疑い**なので、カバレッジ不足に
   混ぜない。多数に及ぶ場合は工程0 の対象照合が漏れた可能性として `run.log` に第一級で記録する。
...
**`sast.enabled: true` のとき**、診断対象マップの「SAST 指摘の根拠」節が仮説の種になる。ただし
**SAST 由来は仮説と根拠にしかならない**（`references/safety-policy.md` ルール11）。SAST が書いた
「確認方法」「再現手順」は決定木の**入力**であって実行計画ではない——実測した SAST 成果物では、
要確認の何件かが外部ネットワークへの送信（8C）・負荷（8B）・サンドボックス脱出 PoC を求めていた。
**各項目を `references/scenario-testing.md` の決定木に通し直して終端する。**「決めない（黙って
落とす）」は禁止。SAST が静的に否定した項目も分母から外さず、DAST で追認する。
...
**工程6の完了条件（無言の打ち切り禁止）**：診断対象マップの**全エントリ**に、該当する
プローブクラスの「実行済みプローブ＋判定、または未実施＋理由」が揃うまで完了としない（高価値プローブ
数本での自己判断の切り上げを禁じる）。**優先度は順序であって絞り込みではない**（低優先も分母に入る）。
`scenarios.md` に「対象×クラス→判定」のカバレッジ行列を残し、
...
**`sast.enabled: true` のとき**は `report.md` に**「SAST 指摘 × DAST 検証」の突合表**を入れる
（`references/report-format.md`）。分母は診断対象マップの「SAST 指摘の根拠」節と「横断・非
エンドポイント指摘」節の**全件**で、工程6 と同じ語彙（確認／否定／要人間／未実施（理由）／
該当なし（理由））で終端し、**同じく無言の打ち切りを禁じる**。突合できなかった指摘は
「未突合」として第一級で残す。

さらに `execution-summary.json`（実行/スキップした工程＋理由、run-id、所要時間）を書く。
`sast.enabled: true` なら **`sast: {run_id, path}`** も入れる（`report: latest` は時点で解決される
ため、`dast.yaml` からはどの SAST run を読んだか復元できない）。
