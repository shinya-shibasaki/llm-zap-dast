...
## 工程6 — シナリオベース診断（`references/scenario-testing.md`、テンプレート `templates/scenario-list.example.md`）

ソース解析＋ZAP履歴をもとに、ZAPの自動スキャンでは見つけにくいシナリオを作成し、ZAP/curl/
Playwright で検証する。対象例：IDOR、水平/垂直権限昇格、認証回避、セッション不備、CSRF、業務
ロジック不備、パラメータ改ざん、Mass Assignment、HTTPメソッド変更、隠しパラメータ変更、
アップロード制御、リダイレクト、APIの認可不足、レート制限不足。

各シナリオに記録：ID、対象機能、想定脆弱性、根拠／前提条件、操作手順／期待される安全な挙動 vs
脆弱時の挙動／実行可否、実行結果、証拠、追加確認事項。

**破壊的なペイロード不可、可用性を損なう検証（DoS相当を含む）不可。**
