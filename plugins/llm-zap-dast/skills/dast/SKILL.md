...
リダイレクト、APIの認可不足、レート制限）と注入系（XSS、SQLi、テンプレート注入、SSRF）の両方。
クラス別のプローブ手順は `references/scenario-testing.md` に従う。
...
（既定OFF）でのみ許可**。`scan.destructive: false` のときは従来どおり検出止まり（悪用・状態変更は
