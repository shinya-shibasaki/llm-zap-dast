...
2. `templates/dast-config.example.yaml` を土台に、検出値で埋めた下書きを作る（コメントは残す）：
   ...
   - `sast`：**既定は `enabled: false`、`report: latest`**。`reports/sast/latest.json` が存在すれば
     「SAST の結果があります。`enabled: true` にすると取り込みます」と**知らせるだけ**にし、
     勝手に true にしない（`true` は「SAST の知見込みの DAST」という約束＝成果物が使えなければ
     run が停止する。利用者が選ぶ）。
     **`--init` は SAST 成果物を読まない。** 読ませると (a) 工程2 の抽出と同じ導出を2経路で持つことに
     なり、(b) 工程0 の安全ゲートより前に対象由来テキストの転記物を読む経路ができる。init が
     知ってよいのは「SAST の結果が存在するか」だけで、中身ではない。
