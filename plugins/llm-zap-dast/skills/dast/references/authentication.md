...
- 標準の読み込み方（値を一切出力しない）——資格情報を使うコマンドの前に同一行で置く：
  ...
  （アカウントごとの `set-credentials`、工程0 で Claude 自身が環境変数の有無を確かめる Bash 等）
  **にこの前置きを毎回付ける**。なお `check_environment.py` は**資格情報の環境変数を検査しない**
  （検査するのは Firefox / Playwright / ZAP 疎通 / `zap.api_key_env` など）ので、そこには不要。
