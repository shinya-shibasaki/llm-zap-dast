...
| `/llm-zap-dast:sast --init` | いま効いている設定を `sast.yaml` に書き出し、`.gitignore` を整備して停止 |
| `/llm-zap-dast:sast --only <step>` | その工程のみ実行（`0` `1` `1'` `2` `3`） |
| `/llm-zap-dast:sast --from <step>` | その工程から工程3まで再開 |

`sast.yaml` は**無くても動きます**（全キー任意。工程0の下見が埋めます）。`--from 2` 以降を使う場合は、
分母にする攻撃マップのパスを指定してください（前回の run を自動では探しません）。

**`--init` は「動かすために必要な設定を作る」ものではありません。** 設定なしで正しく動くのが既定の姿
なので、`sast.yaml` が無くても生成を提案することはありません（必須キーを持つ `dast.yaml` とは事情が
違います）。`--init` の役割は、**いまどの設定で動いているのかを読める形でリポジトリに残す**ことです
——既定のまま効いている値も**省略せずに書き出す**ので、ファイルを見れば現在の挙動が分かります。
あわせて `.gitignore` に出力先と `.env` を追記するので、**通常 run 側の `.gitignore` 同意停止が
起きなくなります**（初回だけ run の途中で確認が入るのを避けたい場合に便利です）。

生成物では **`tools.semgrep.configs` を固定しません**（コメントのまま）。パックは毎回 profiling が
検出言語から選ぶ運用で、固定すると言語構成が変わったときに古い選定が残り続けるためです。実際に
使ったパックは毎回レポートの「実施したスキャン設定」に記録されるので、事後に追えます。
...
`report.md` は**セキュリティに詳しくない人が読んで対応を判断できる**ことを基準に書きます — 冒頭に
「今回わかったこと」を専門語なしで数行、検出のタイトルは平易な一文（技術名は括弧で併記）、リスクは
「誰が・何をされると・何が起きるか」の3点、修正案には「直すと何が防げるか」を1行。監査のための短い
ラベル（事実の出所・確度・8A/8B/8C など）は表では短いまま残し、冒頭の「凡例」ブロックで言い換えます。
